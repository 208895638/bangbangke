...
class ListBox extends AjaxDisplayObject implements IFormControls {
    ...
    constructor(jq: JQuery, cfg: any) {
        ...
        if (jq[0].tagName !== 'SELECT') {
            jq = $('<select name="' + cfg.name + '" class="form-control"></select>').appendTo(jq);
            if (cfg.multiple) {
                jq.attr('multiple', true)
            }
        } else {
    ...
    bindHandler(json) {
        ...
        if (this.autoPrependBlank) {
            let txt = (typeof this.autoPrependBlank === 'string') ? this.autoPrependBlank : '--';
    ...
    public set selectedValue(v) {
       let $items = $(this._items).removeAttr('selected');
        $items.parent().val(v);
    }
...
class CheckBox extends AjaxDisplayObject implements IFormControls {
    ...
    constructor(jq: JQuery, cfg: any) {
        ...
        cfg = $.extend({
            bindOptions: {
                ...
                template: '<label class="t-checkbox"><input class="t-checkbox__inner" name="' + cfg.name
                + '" type="checkbox" value="${' + (cfg.value || 'id') + '}"><span class="fa t-checkbox__label"></span>${' + (cfg.text || 'name') + '}</label>'
                + (cfg.joiner === undefined ? ' ' : cfg.joiner)
            }
        }, cfg);
        ...
        for (let i = 0, l = arr.length; i < l; i++) {
            ...
            if (item.length) {
                ...
                if (!item.prop('checked')) {
                    chkItem.push(item.attr('checked', true));
                }
    ...
    get unSelectedData(): Array {
        let s = this._items.filter(':not(:checked)'),
            arr = [],
            that = this;
    ...
    setDisabled(index, disabled: boolean = true) {
        let item = this.jq.find('.t-checkbox').eq(index);
        if (item) {
            if (!disabled) {
...
class RadioBox extends AjaxDisplayObject implements IFormControls {
    ...
    constructor(jq: JQuery, cfg: any) {
        ...
        cfg = $.extend({
            bindOptions: {
                ...
                // + (cfg.joiner === undefined ? ' ' : cfg.joiner)
                template: '<label class="t-radio"><input  class="t-radio__inner"  name="' + cfg.name
                + '" type="radio" value="${' + (cfg.value || 'id') + '}"><span class="t-radio__label"></span>${' + (cfg.text || 'name') + '}</label>'
                + (cfg.joiner === undefined ? ' ' : cfg.joiner)
            }
        }, cfg);
    ...
    setDisabled(index, disabled: boolean = true) {
        let item = this.jq.find('.t-radio').eq(index);
        if (item) {
            if (!disabled) {
                item.removeClass('is-disabled');
            } else {
