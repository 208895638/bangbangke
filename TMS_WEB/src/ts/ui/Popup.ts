...
const DEFAULTS = {
    ...
    modalClose: false,          // click dialog modal can close ?
...
function setTitleBar(cfg) {
    ...
    $('<div class="pop-row tb-row" />').prependTo(this.boxy).append(tb);
...
function setFooter(cfg) {
    ...
    for (let key in cfg.buttons) {
        ...
        htmlArr.push(`<button class="btn ${cls}" name="${x}" data-key="${key}">${txt}</button>`);
    }
    ...
    $('<div class="pop-row tf-row" />').appendTo(this.boxy).append(footer);
}
...
class PopUp extends DisplayObject {
    ...
    static confirm(message, callback, options = {}): PopUp {
        ...
        if (!cfg.buttons.cancel) {
            cfg.buttons.cancel = `${Languages.package.cancel}`;
        }
...
function wrapAsk(msg, cb, cfg) {
    ...
    if (!cfg.buttons.ok) {
        cfg.buttons.ok = `${Languages.package.ok}`;
    }
