...
let gridEditrules = {
    required(value, colname, nm) {
        if (!value) {
            // window.alert = opg.err;
            // setTimeout(() => {
            //     window.alert = alert;
            // }, 10);
            return [false, `${colname} 字段不能为空`];
        }
        return [true, ''];
    }
};
...
export {is, url, request, string, dateTime, convert, format, cleanValueObject, array, gridEditrules};